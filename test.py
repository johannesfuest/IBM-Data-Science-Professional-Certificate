...
def get_pie_chart(site_dropdown):
    ...
    if site_dropdown== 'ALL':
        ...
        filtered_df = spacex_df.loc[spacex_df['Launch Site'] == site_dropdown].groupby(['Launch Site', 'class']).size().reset_index(name='class_count')

        fig = px.pie(filtered_df, values = 'class_count',
        names='class',
        title='Share of Success Launches')
        return fig
...
def get_scatter_chart(site_dropdown, payload_slider):
    ...
    filtered_df1 = filtered_df1.loc[(filtered_df1['Payload Mass (kg)'] >= payload_slider[0]) & (filtered_df1['Payload Mass (kg)'] <= payload_slider[1])]
    if site_dropdown == 'ALL':
        fig = px.scatter(filtered_df1, x= 'Payload Mass (kg)', y = 'class', color="Booster Version Category")
        return fig
    else:
        filtered_df1 = spacex_df.loc[spacex_df['Launch Site'] == site_dropdown]
        filtered_df1 = filtered_df1.loc[(filtered_df1['Payload Mass (kg)'] >= payload_slider[0]) & (filtered_df1['Payload Mass (kg)'] <= payload_slider[1])]
        fig = px.scatter(filtered_df1, x= 'Payload Mass (kg)', y = 'class', color="Booster Version Category")
        return fig
# Run the app
